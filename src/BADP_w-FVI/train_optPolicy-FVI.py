...
value_model = ExtraTreesRegressor(n_estimators=100, random_state=seed)

# State = (R_start, x_start, )
...
for t_i in range(T - 1, -1, -1):
    ...
    if t_i < T - 1:
        ...
        P_intraday_sample_next = P_intra_state[:,
                                               t_i + 1, :].reshape(N, D * 24 * 4)
    ...
    for n in range(N):
        ...
        if t_i < T - 1:
            ...
            if lk > 1:
                ...
                for i in range(1, lk):
                    ...
                    VR_abc_neg[i - 1, 0] = VR[i, 1] - \
                        VR_abc_neg[i - 1, 1] * R_k[i]
                ...
                for i in range(1, lk):
                    ...
                    VR_abc_pos[i - 1, 0] = VR[i, 1] - \
                        VR_abc_pos[i - 1, 1] * R_k[i]
        ...
        for iR in range(length_R):
            ...
            for ix in range(len(x_vec)):
                ...
                q_pump_up = (abs(mu_intraday) / Q_mult -
                             Q_fix) * t_ramp_pump_up / 2
                q_pump_down = (abs(mu_intraday) * Q_mult +
                               Q_fix) * t_ramp_pump_down / 2
                q_turbine_up = (
                ...
                A, b, Aeq, beq, lb, ub = linear_constraints_train_FVI(
                    Delta_ti,
                    beta_pump,
                    beta_turbine,
                    c_pump_up,
