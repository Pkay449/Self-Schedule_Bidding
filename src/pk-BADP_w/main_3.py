...
def compute_weights(eng, phi, Y, weights_vector):
    ...
    w_row = matlab.double(weights_vector.tolist())
    w_mat = eng.VRx_weights(phi_mat, Y_mat, w_row)
    return np.array(w_mat).flatten()
...
# Forward pass (Similar logic)
# Generate forward scenarios
# After the backward pass is done and Vt is computed, we proceed with the forward pass.
...
for m in range(M):
    ...
    for t_i in range(T):
        ...
        A = np.vstack([A1, A2, A3, A4, AV_neg, AV_pos])
        b = np.concatenate([b1, b2, b3, b4, bV_neg, bV_pos]) if lk>1 else np.concatenate([b1,b2,b3,b4])
        ...
        #R_hq
        A1=[-np.eye(96)+np.diag(np.ones(95),-1),
            np.zeros((96,96)),
            Delta_ti*beta_pump*np.eye(96),
            -Delta_ti/beta_turbine*np.eye(96),
            -beta_pump*c_pump_up*np.eye(96),
        ...
        A1=[np.zeros((96,96*2)),
            ...
            np.zeros((96,96*3+24)),
            np.zeros((96,1)),
            np.zeros((96,96,))] # carefully build same as MATLAB
        ...
        x_opt2, fval2 = build_and_solve_intlinprog(eng, f, A, b, Aeq, beq, lb, ub, intcon, intlinprog_options)
