...
% % Assuming P_day_state and P_intra_state have shape (N, T, 24*D)
% disp('First and last element of P_day_state:');
% disp(P_day_state(1, 1, :)); % First element
% disp(P_day_state(end, end, :)); % Last element
%
% disp('First and last element of P_intra_state:');
% disp(P_intra_state(1, 1, :)); % First element
% disp(P_intra_state(end, end, :)); % Last element
...
for t=T:-1:1 %Backwards step
    ...
    for n=1:N
        ...
        if t<T
            ...
            % points=[R_vec',VRx(:,2)];
            % if size(points, 1) < 3
            %     isCollinear = true;  % Less than 3 points are always collinear
            % else
            %     % Compute the rank of the matrix formed by subtracting the first point
            %     rank_value = rank(points - points(1, :));
            %     isCollinear = rank_value < 2;  % If rank < 2, points are collinear
            % end
            % if isCollinear
