function [ EV ] = clean_badp_w(N,M,T,Season,length_R,seed)
...
% Set Data folder relative to current directory
...
weights_D_value=clean_badp_weights(T);
% save weights_D_value as weights_D_value_1.mat
save(fullfile(dataDir, 'weights_D_value_1.mat'), 'weights_D_value');
...
load(fullfile(dataDir, strcat('P_day_', Season, '.mat')));
load(fullfile(dataDir, strcat('P_intraday_', Season, '.mat')));
...
for n=1:N
    ...
    for t=1:T
        ...
        [mu_day,cor_day]=clean_sample_price_day(P_day,t,Season);
        Wt_day=mvnrnd(mu_day,cor_day);
        
        [mu_intraday,cor_intraday]=clean_sample_price_intraday([Wt_day,P_day],P_intraday,t,Season);
...
% save P_day as P_day_1.mat and P_intraday as P_intraday_1.mat
save(fullfile(dataDir, 'P_day_1.mat'), 'P_day');
save(fullfile(dataDir, 'P_intraday_1.mat'), 'P_intraday');
...
for t=T:-1:1 %Backwards step
    ...
    for n=1:N
        ...
        mu_day=clean_sample_price_day(P_day,t,Season);
        mu_intraday=clean_sample_price_intraday([mu_day,P_day],P_intraday,t,Season);
        ...
        if t<T
            ...
            weights = clean_VRx_weights(phi, Y, weights_D_value(t+1,:) );
...
for m=1:M
    ...
    C=0;
    for t=1:T
        [mu_day,~]=clean_sample_price_day(P_day_sim,t,Season);
        [mu_intraday,~]=clean_sample_price_intraday([mu_day,P_day_sim],P_intraday_sim,t,Season);
        ...
        [mu_intraday,~]=clean_sample_price_intraday([Wt_day,P_day_sim],P_intraday_sim,t,Season);
